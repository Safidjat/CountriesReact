...
function Main() {
    ...
    function axtar(){
        const newAr= zapaz.filter(item=>item.name.official.toLowerCase().includes(text))
        setData(newAr)
    }
console.log(data);
        ...
        <>
            <section className="bg-gray-800 dark:bg-gray-100 text-gray-100 dark:text-gray-800">
                <div className={`${!gizlet ? 'h-0': 'h-[108px]'} transition-all duration-200 ease-in-out overflow-hidden`}>
                    <div className="relative pt-[50px] m-auto max-w-[394px]  flex  gap-2 p-4">
                        ...
                        <button onClick={()=>{axtar();scrollToId(document.getElementById('content'))}} className='cursor-pointer hover:bg-[#ccc] p-[10px] dark:bg-[#6366f1] text-[#1e2939] rounded-lg dark:hover:bg-[#4345a4] bg-white dark:text-white'><FaSearch /></button>
                    </div>
                </div>
            ...
            <section className="bg-gray-800 dark:bg-gray-100 text-gray-100 dark:text-gray-800">
                <div className="container max-w-6xl p-6 mx-auto space-y-6 sm:space-y-12">
                    {data.length > 0 && <RandomCard {...zapaz[randomIndex]} />}
